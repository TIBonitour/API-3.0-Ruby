...
describe Cielo::API30::Sale do
  ...
  describe "#as_json" do
    it "should have the same value" do
      ...
      subject.fraud_analysis = fraude
      expect(subject.as_json.has_key?(:MerchantOrderId)).to be_truthy
      expect(subject.as_json.has_key?(:Customer)).to be_truthy
      expect(subject.as_json.has_key?(:Payment)).to be_truthy
      expect(subject.as_json.has_key?(:FraudAnalysis)).to be_truthy
    end
  end
end